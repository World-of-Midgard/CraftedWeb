...
*/
    ...
    else
    {
        ...
        $result = $sql->query("SELECT name FROM realms WHERE id='".$realm_id."'");
        $row = mysqli_fetch_assoc($result);
        $realm = $row['name'];
        connect::connectToRealmDB($realm_id);
        $result = $sql->query("SELECT name,guid,gender,class,race,level,online FROM characters WHERE guid='".$guid."'");
        $row = mysqli_fetch_assoc($result)
        ?>
